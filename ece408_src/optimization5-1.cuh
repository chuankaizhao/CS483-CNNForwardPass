...
__global__ void matrixMultiplyShared1(float *Kernel, float *X, float *Y, int M, int C, int H, int W, int K, int H_out, int W_out) 
{
  ...
  int tx = threadIdx.x;
  int ty = threadIdx.y;
  int rowIx = blockIdx.y * TILE_WIDTH_1 + ty;
  int colIx = blockIdx.x * TILE_WIDTH_1 + tx;

  float result = 0;
  int q, p, c, w, h;

  for (int tileIx = 0; tileIx < ceil(1.0*numAColumns/TILE_WIDTH_1);  ++tileIx) {
...
__global__ void matrixMultiplyShared2(float *Kernel, float *X, float *Y, int M, int C, int H, int W, int K, int H_out, int W_out)
{
  //@@ Insert code to implement matrix multiplication here
  //@@ You have to use shared memory for this MP
  int numAColumns = C*K*K;
  ...
  for (int tileIx = 0; tileIx < ceil(1.0*numAColumns/TILE_WIDTH_2);  ++tileIx) {
...
void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y, const mshadow::Tensor<gpu, 4, float> &x, const mshadow::Tensor<gpu, 4, float> &k)
{
    ...
    if (M / 24){
        ...
        int gridDimY = ceil(1.0*M/TILE_WIDTH_1), gridDimX = ceil(1.0*H_out*W_out/TILE_WIDTH_1);
